...
class YOLO(object):
    _defaults = {
        ...
        "classes_path"      : 'model_data/npustspeedbump.txt',#目標物體標籤文件
        "model_image_size"  : (416, 416, 3),#輸入層大小
        "confidence"        : 0.5,#超過這個置信度的值物體才能被模型偵測出來
        "iou"               : 0.01,#兩個結果大於這個值就可以執行nms
        "cuda"              : True,
        #---------------------------------------------------------------------#
    ...
    #   生成模型
    #---------------------------------------------------#
    def generate(self,epoh_num):
        new_model_path ="logs/Epoch"+str(epoh_num)+".pth"
        print("hihi",new_model_path)
        ...
        print('{} model, anchors, and classes loaded.'.format(new_model_path))
        # 画框设置不同的颜色
